...
class TransferQueue {
    ...
    func addFileDownload(_ fileDownload: FileDownload) {
        let throttler = Throttler(interval: 0.5)
        
        let name = fileDownload.displayName
        ...
        Task {
            ...
            fileDownload.progressHandler = { (state) in
                Task {
                    await throttler.throttle {
                        await MainActor.run { [weak self] in
                            ...
                            if let index = self.transfers.firstIndex(where: { $0.id == id }) {
                                self.transfers[index] = TransferInfo(id: id, name: name, state: state)
                                NotificationCenter.default.post(name: TransferQueue.progressDidChange, object: self)
                            }
                        }
                    }
                }
