...
class FilesViewController: NSViewController {
    ...
    @IBAction func downloadFileAction(_ sender: Any?) {
        ...
        presentSavePanel(for: fileNode)
    }
    
    private func presentSavePanel(for fileNode: FileNode) {
        let savePanel = NSSavePanel()
        savePanel.title = "Save File"
        savePanel.nameFieldStringValue = fileNode.name
        
        savePanel.begin { result in
            if result == .OK, let destination = savePanel.url {
                self.startDownload(fileNode: fileNode, to: destination)
            }
        }
    }
    
    private func startDownload(fileNode: FileNode, to destination: URL) {
        let fileDownload = FileDownload(source: fileNode.path, destination: destination, accessor: treeAccessor)
        TransferQueue.shared.addFileDownload(fileDownload)
    }
...
extension FilesViewController: NSMenuItemValidation {
    func validateMenuItem(_ menuItem: NSMenuItem) -> Bool {
        ...
        case #selector(renameContextMenuAction(_:)):
            ...
            return true
        case #selector(downloadFileAction(_:)):
            guard targetRows.count > 0 else { return false }
            return targetRows.allSatisfy { outlineView.item(atRow: $0) is FileNode }
        default:
            return false
